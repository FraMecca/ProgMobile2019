...
int transcoding(BufferData *p_dst_buf, int *out_bit_rate, float *out_duration, const TranscodingArgs args, const BufferData src_buf, size_t start_time, size_t end_time)
{
    ...
    /* av_seek_frame(input_format_context, -1, 50000000, AVSEEK_FLAG_ANY); */ // TODO convert time to framse
    ...
    while (1)
    {
        int nseconds = pts / output_codec_context->sample_rate;
        if(nseconds > 50) break;
        else printf("N:%ld %d, %d, %d\n", pts, nseconds, output_codec_context->frame_number, output_codec_context->sample_rate);
